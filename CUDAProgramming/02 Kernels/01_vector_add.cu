...
int main(){
    ...
    int grid_1d=(N+BLOCK_SIZE_1D-1)/BLOCK_SIZE_1D;
    ...
    dim3 grid_3d(
       ...
       (nx + block_size_3d.y -1)/block_size_3d.y,
       (nx + block_size_3d.z -1)/block_size_3d.z
    );
    ...
    for(int i=0;i<3;i++){
        ...
        vector_add_gpu_1d<<<grid_1d,BLOCK_SIZE_1D>>>(d_a,d_b,d_c_1d,N);
        vector_add_gpu_3d<<<grid_3d,block_size_3d>>>(d_a,d_b,d_c_3d,nx,ny,nz);
    ...
    for(int i=0;i<100;i++){
        ...
        double start_time=get_time();
        vector_add_gpu_3d<<<grid_3d,block_size_3d>>>(d_a,d_b,d_c_1d,nx,ny,nz);
        cudaDeviceSynchronize();
        double end_time=get_time();
